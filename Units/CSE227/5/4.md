...
class WifiSpeedTest : AppCompatActivity() {
    ...
    Thread {
        try {
            ...
            val bytes = URL(urlString).openStream().readBytes().size
            val duration = (System.currentTimeMillis() - start) / 1000.0
            val speed = (bytes * 8) / (duration * 1024 * 1024)
            runOnUiThread {
                speedTextView.text = "Download Speed: %.2f Mbps".format(speed)
            }
        } catch (_: Exception) {
            runOnUiThread { speedTextView.text = "Error testing speed" }
        }
    }.start()
}
```